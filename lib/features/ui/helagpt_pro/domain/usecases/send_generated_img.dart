  ...
  Future<File> generateImageFromText(String text) async {
    ...
    final response = await http.post(
      ...
      headers: {
        ...
        'Authorization': 'Bearer $hugapiKey', // Use the apiKey parameter
      },
      body: jsonEncode({
        'inputs': translatedText.text, // Use the translated text
      }),
    );

    if (response.statusCode == 200) {
      // The response is expected to be binary image data
      Uint8List bytes = response.bodyBytes; // Get the raw bytes
